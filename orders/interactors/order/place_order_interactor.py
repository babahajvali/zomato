...
class PlaceOrderInteractor(PromoCodeMixin):
    ...
    def place_order(self, order_data: PlaceOrderDTO) -> OrderDTO:
        ...
        with redis_lock(f"order_{order_data.customer_id}", timeout=10):
            if order_data.promo_code_id:
                with redis_lock(f"promo_{order_data.promo_code_id}", timeout=10):
    ...
    def _validate_promo_code_available(
        self,
        promo_code_id: int,
        max_usage_count: int,
    ):
