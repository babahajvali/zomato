...
def map_scheduled_order_response(order_dto) -> ScheduledOrderSummaryType:
...
def map_scheduled_orders_response(order_dtos) -> ScheduledOrderSummariesType:
...
def get_today_restaurant_scheduled_orders_resolver(root, info, params):
    interactor = GetRestaurantOrderInteractor(order_storage=OrderStorage())

    try:
        order_dtos = interactor.get_today_restaurant_scheduled_orders(
            restaurant_id=params.restaurant_id,
            user_id=info.context.user_id,
            limit=params.limit,
            offset=params.offset,
