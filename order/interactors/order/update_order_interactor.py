...
class UpdateOrderStatusInteractor(OrderMixin):
    ...
    def update_order_status(self, order_id: str, status: OrderStatus, user_id: str) -> OrderDTO:
        order_dto = self._get_validated_order(order_id=order_id)
        self._validate_ownership_and_transition(order_dto=order_dto, user_id=user_id, status=status)
    ...
    def _validate_ownership_and_transition(self, order_dto: OrderDTO, user_id: str, status: OrderStatus):
