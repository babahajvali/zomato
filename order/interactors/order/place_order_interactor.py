...
from order.interactors.storage_interface.order_storage_interface import OrderStorageInterface
...
class PlaceOrderInteractor(PromoCodeMixin):
    ...
    def place_order(self, order_data: PlaceOrderDTO) -> OrderDTO:
        cart_id = self.restaurant_adapter.get_customer_cart_id(customer_id=order_data.customer_id)
    ...
    def _validate_promo_code_existence_and_eligibility(self, promo_code_id: int, items_total: float):
        self.validate_promo_code_exist(promo_code_id=promo_code_id)
        promo_code_dto = self.promo_code_storage.get_promo_code_by_id(promo_code_id=promo_code_id)
    ...
    def _validate_address_and_get_delivery_fee(self, address_id: int, restaurant_id: str) -> float:
        pincode = self._get_validated_pincode(address_id=address_id)
        return self._get_validated_delivery_fee(restaurant_id=restaurant_id, pincode=pincode)
    ...
    def _get_validated_delivery_fee(self, restaurant_id: str, pincode: str) -> float:
        ...
        if zone_dto is None:
            raise InvalidDeliveryZoneFound(restaurant_id=restaurant_id, pin_code=pincode)
    ...
    def _execute_order(
        ...
        tax_fee = self._calculate_tax_fee(items_total=items_total, discount_price=discount_price)
    ...
    def _get_discount_price(self, promo_code_id: int | None, items_total: float) -> float:
    ...
    def _validate_promo_code_available(self, promo_code_id: int, max_usage_count: int):
        usage_count = self.order_storage.get_promo_code_usage(promo_code_id=promo_code_id)
    ...
    @staticmethod
    def _calculate_discount_price(items_total: float, discount_type: str, discount_value: float) -> float:
    ...
    @staticmethod
    def _build_order_items(cart_items: List[CartItemDTO], order_id: str) -> List[CreateOrderItemDTO]:
        return [
            CreateOrderItemDTO(
                order_id=order_id,
