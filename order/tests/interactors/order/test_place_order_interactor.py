...
from order.adapter.dtos import (
    AddressDTO,
    CartItemDTO,
    DeliveryZoneDTO,
    RestaurantTimingDTO,
...
from order.tests.factories.dto_factories import (
    OrderDTOFactory,
...
class TestPlaceOrderInteractor:
    ...
    def _setup_valid_adapters(self):
        ...
        self.interactor.restaurant_adapter.get_delivery_zone_by_restaurant_id.return_value = (
            DeliveryZoneDTO(
                delivery_zone_id=1,
                restaurant_id="restaurant-1",
                pin_code="500001",
    ...
    @patch(
        "order.interactors.order.place_order_interactor.transaction.atomic",
        no_op_lock,
    )
    @patch("order.interactors.order.place_order_interactor.redis_lock", no_op_lock)
    ...
    def test_place_order_raises_restaurant_day_timing_not_found(self):
        ...
        with pytest.raises(RestaurantDayTimingNotFound):
    ...
    def test_place_order_raises_invalid_address_found(self):
        ...
        with pytest.raises(InvalidAddressFound) as exc:
            self.interactor.place_order(
                order_data=PlaceOrderDTOFactory(address_id=999)
            )
    ...
    def test_place_order_raises_invalid_delivery_zone_found(self):
        ...
        with pytest.raises(InvalidDeliveryZoneFound) as exc:
            self.interactor.place_order(
                order_data=PlaceOrderDTOFactory(restaurant_id="restaurant-1")
            )
