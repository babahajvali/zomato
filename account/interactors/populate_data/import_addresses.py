...
from account.exception.custom_exceptions import AlreadyExistsAddress, DuplicateAddresses
...
class ImportAddresses:
    ...
    def import_addresses(self, file_path="./sample_data/addresses.csv"):
        ...
        self._validate_duplicate_addresses(email_label_pairs)
        self._validate_existing_addresses(email_label_pairs)
    ...
    @staticmethod
    def _validate_rows_and_get_email_and_label(
        rows: list[dict[Any, str | Any]],
    ) -> list[Any]:
        email_label_pairs = []
        for index, row in enumerate(rows, start=1):
            validate_row(
                row, ["email", "label", "full_address"], f"address row {index}"
            )
    ...
    def _validate_existing_addresses(self, email_label_pairs: List[tuple]):
        emails = [pair[0] for pair in email_label_pairs]
        labels = [pair[1] for pair in email_label_pairs]

        existing_addresses = self.address_storage.get_existing_addresses(emails, labels)

        if existing_addresses:
            raise AlreadyExistsAddress(addresses=existing_addresses)

    @staticmethod
    def _validate_duplicate_addresses(email_label_pairs: List[tuple]):
        seen = set()
        duplicates = []
