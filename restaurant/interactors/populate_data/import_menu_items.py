...
class ImportMenuItems:
    ...
    def import_menu_items(self, file_path="./sample_data/menu_items.csv"):
        ...
        self._validate_restaurants_exist(restaurant_ids)
    ...
    def _validate_restaurants_exist(self, restaurant_ids: List[str]):
        existing_ids = self.restaurant_storage.get_restaurants_by_ids(
            restaurant_ids
        )

        existing_ids_set = set(existing_ids)
        missing_ids = [rid for rid in restaurant_ids if
                       rid not in existing_ids_set]
